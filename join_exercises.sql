...
SELECT 
    u.name AS UName,
    r.name AS RName,
    COUNT(r.name) AS 'Role Count'
FROM
    users AS u
        RIGHT JOIN
    roles AS r ON u.role_id = r.id
GROUP BY u.name , r.name , 'Role Count';
...
SELECT	* FROM departments;
SELECT 
    d.dept_no, d.dept_name, COUNT(emp_no) AS num_employees
FROM
    dept_emp AS de
        JOIN
    departments AS d ON de.dept_no = d.dept_no
...
SELECT 
    d.dept_name, ROUND(AVG(s.salary), 0) AS avg_dept_salary
FROM
    departments d
        JOIN
